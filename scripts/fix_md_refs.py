...
遇到新问题时的排查思路：
  1. python scripts/fix_md_refs.py <file> --dry-run 看 diff
  2. 用 --diagnose 模式查看文件中各类引用模式的统计
  3. 检查是哪个 pipeline 阶段遗漏，对应修改

Usage:
  python scripts/fix_md_refs.py <file_or_dir> [--dry-run] [--diagnose]
"""
...
def fix_markdown(text: str) -> str:
    """Apply all citation fixes."""
    # 0. Merge fragmented Marker citation links into clean [N] text
    text = merge_fragmented_citations(text)

    # 0b. Fix translated citations with #page-XX-X anchors
    text = fix_page_anchor_citations(text)

    # 1. Remove escaped parens
    text = fix_escaped_parens(text)

    # 2. Parse references
    number_map = _parse_numbered_refs(text)
    ay_map = _parse_author_year_refs(text)

    # 3. Convert body citations to links
    text = convert_numbered_citations(text, number_map)
    text = convert_author_year_citations(text, ay_map)

    # 4. Add anchors in References section
    text = add_ref_anchors(text, number_map, ay_map)

    # 5. Fix spacing
    text = fix_bracket_spacing(text)
