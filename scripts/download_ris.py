...
OUT = ROOT / "scripts" / "attention_is_all_you_need.ris"
...
def pass_security_check(page: Page, timeout: int = 90) -> bool:
    ...
    page.screenshot(path=str(ROOT / "scripts" / "cloudflare_timeout.png"), full_page=True)
...
def download_ris(page: Page) -> bool:
    ...
    page.screenshot(path=str(ROOT / "scripts" / "after_security.png"), full_page=True)
...
def main() -> None:
    with sync_playwright() as p:
        ...
        if not download_ris(page):
            page.screenshot(path=str(ROOT / "scripts" / "ris_not_found.png"), full_page=True)
